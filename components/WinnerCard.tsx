...
function winnerLabel(winner?: Speaker | "draw") {
  if (winner === "draw") return "The court finds a draw";
  return winner ? `Judgment for Counsel ${winner}` : "Judgment pending";
}
...
export function WinnerCard({ session, loading, onReplay }: WinnerCardProps) {
  ...
  return (
    <section className={`winner-card winner-card--${winner ?? "pending"}`} aria-live="polite">
      ...
      <span className="section-kicker">Final judgment</span>
      ...
      {loading ? (
        <div className="verdict-loading"><i /><i /><i /> Writing the opinion</div>
      ) : (
        <p>{session.verdict}</p>
      )}
      <button type="button" className="secondary-button" onClick={onReplay}>
        <RotateIcon className="h-4 w-4" /> Hear another case
      </button>
    </section>
  );
}
